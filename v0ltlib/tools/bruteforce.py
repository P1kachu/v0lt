...
class Bruteforce:
    ...
    def __init__(self, allowed_chars, length, begin_with="", end_with=""):
        self.dictionnary = allowed_chars
        self.range_limit = len(self.dictionnary)
        self.begin_with = begin_with
        self.end_with = end_with + "\n"
        self.length = length - (len(self.begin_with) + len(self.end_with) - 1)
        if len(allowed_chars) > length:
